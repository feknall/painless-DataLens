...
# core method from 'Scalable Private Learning with PATE' 
...
def compute_logq_gaussian(counts, sigma):
  ...
  # print(multivariate_normal.cdf(
  #   counts_rest, np.zeros(n - 1), covariance, maxpts=1e12))
  # logq = np.log1p(-multivariate_normal.cdf(
  #     counts_rest, np.zeros(n - 1), covariance, maxpts=1e12))
  # print("log q 2:", logq)
  # print("another:", math.log(1 - (1 / n)))
...
def double_rdp_gaussian(logq, sigma, orders):
  """Bounds RDP from above of GNMax given an upper bound on q (Theorem 6).
  ...
  if (np.any(mask) and logq <= log_a2 - mu_hi2 *
    ...
    log_s = np.logaddexp(log1q + log_a, logq + log_b)
    print('choose data dependent bound: ', np.sum((ret > log_s / (orders - 1))[mask]))
    # always use data-dependent bound without comparison
    ret1[mask] = (log_s / (orders - 1))[mask]
  else:
    print("Condition not satisfied")
  ...
  if np.isscalar(orders):
    return np.asscalar(ret1), np.asscalar(ret)
  else:
    return ret1, ret
